...
def max_subseq(n, t):
    ...
    def digit_of_biggest(x):
        if x % 10 == find_biggest(x):
            ...
            return 1 + digit_of_biggest(x // 10)

    if t == 0:
        return 0
    if t == 1:
        return find_biggest(n)
    else:
        to_select = n // (10 ** (t - 1))
        biggest_this_turn = find_biggest(to_select)
        digit_of_biggest_in_all = digit_of_biggest(to_select) + t - 1
        changed_n = n % (10 ** (digit_of_biggest_in_all-1))
        return biggest_this_turn * (10 ** (t - 1)) + max_subseq(changed_n, t-1)
    '''
    if t == 0:
        return 0
    elif n < 10:
