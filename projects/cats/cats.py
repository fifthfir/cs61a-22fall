...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        for valid_word in valid_words:
            diff1 = diff_function(user_word, valid_word, limit)
            if diff1 <= limit and diff1 < diff0:
                that_word = valid_word
                diff0 = diff1
        return that_word
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    if limit < 0:
        ...
        if start[0] == goal[0]:
            ...
            step = 1
            limit -= 1
        return step + shifty_shifts(start[1:], goal[1:], limit)
    # END PROBLEM 6
...
def final_diff(start, goal, limit):
    ...
    if start[0] == goal[0]:
        ...
        remove_diff = start[1:]
        substitute_diff = goal[0] + start[1:]
        adjacent_diff = start[1] + start[0] + start[2:]
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            time = times_per_player[i][j + 1] - times_per_player[i][j]
            time_i.append(time)
        times.append(time_i)
