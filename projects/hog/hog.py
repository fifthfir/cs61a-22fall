...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    start = min(player_score, opponent_score)
    divided = max(player_score, opponent_score)
    a = start
    while a >= 10:
        if start % a == 0 and divided % a == 0:
            return True
        else:
            a -= 1
    return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            score_oppo = score1
            score0 += take_turn(strategy0(score0, score1), score_oppo, dice)
            score = score0
        else:
            score_oppo = score0
            score1 += take_turn(strategy1(score1, score0), score_oppo, dice)
            score = score1
        if not extra_turn(score, score_oppo):
            who = other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            score = score1
        gain_this_turn = score - last_score
        if gain_this_turn > running_high:
            print(str(gain_this_turn) + ' point(s)! The most yet for Player ' + str(who))
            running_higher = gain_this_turn
        else:
            running_higher = running_high
        return announce_highest(who, score, running_higher)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while i <= 10:
        max_score_i = average_dice(i, dice)
